...
namespace @event.server.csharp
{
    public class ConnectionManager: Subsciption.Iface
    {
        ...
        {
            return new UserData(user.Name, user.Password) {Id = user.GetId()};
        }
        ...
        {
            return new PlayerData(player.Name, player.Age) {Id = player.GetId()};
        }
        ...
        {
            return new EventData(element.Distance, element.Style.ToString()) {Id = element.GetId()};
        }
